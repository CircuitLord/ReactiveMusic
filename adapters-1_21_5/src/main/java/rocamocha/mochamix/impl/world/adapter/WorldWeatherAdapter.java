...
public class WorldWeatherAdapter implements Weather {
    ...
    public boolean isRainingAt(MinecraftVector3 pos) {
        ...
        return w.isRaining() && w.getBiome(blockPos).value().getPrecipitation(blockPos, w.getSeaLevel()) == Biome.Precipitation.RAIN;
    }
    ...
    public boolean isSnowingAt(MinecraftVector3 pos) {
        BlockPos blockPos = (BlockPos) pos.asNativeBlockPos();
        return w.isRaining() && w.getBiome(blockPos).value().getPrecipitation(blockPos, w.getSeaLevel()) == Biome.Precipitation.SNOW;
    }
}
